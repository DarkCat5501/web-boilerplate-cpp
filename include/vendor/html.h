...
class fattr : public attr{
 public:
    fattr(const char *name) : attr(name,"") { }
    fattr(const std::string& name) : attr(name,"") { }
    friend std::ostream& operator<<(std::ostream& os, const class fattr& a) {
        return os << a.name_;
    }
};
